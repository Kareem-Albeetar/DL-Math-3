...
def test_fit_and_predict_regression_model_1d():
    ...
    expected_values = torch.tensor([[1252.3008], [939.9971], [627.6935]])
    
    print("Actual prediction:")
    print(y_pred)
    
    differences = (y_pred - expected_values).abs()
    print("Differences:")
    print(differences)
    
    assert (differences < 50).all(), "y_pred is not correct"
    assert y_pred.shape == (3, 1), "y_pred shape is not correct"

    # Print training loss and model parameters after training
